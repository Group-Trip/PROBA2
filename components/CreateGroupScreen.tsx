    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="p-4 space-y-4 pb-8">
            ...
            <SelectContent>
              {Array.from({ length: 15 }, (_, i) => i + 1).map((num) => (
                <SelectItem key={num} value={num.toString()}>
                  {num} {num === 1 ? 'bilet' : num <= 4 ? 'bilety' : 'biletów'}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
          {numberOfTickets !== "" && (
            <div className="mt-3 p-3 bg-blue-50 rounded-lg">
              <p className="text-sm text-blue-900">
                📊 Zajmujesz <strong>{totalOccupiedSpots} {totalOccupiedSpots === 1 ? 'miejsce' : totalOccupiedSpots <= 4 ? 'miejsca' : 'miejsc'}</strong> z {TOTAL_SLOTS}
              </p>
              <p className="text-sm text-blue-700 mt-1">
                🎯 Pozostało <strong>{TOTAL_SLOTS - totalOccupiedSpots} {TOTAL_SLOTS - totalOccupiedSpots === 1 ? 'miejsce' : TOTAL_SLOTS - totalOccupiedSpots <= 4 ? 'miejsca' : 'miejsc'}</strong> dla innych
              </p>
              {selectedAttraction && (
                <p className="text-sm text-green-700 mt-2">
                  💰 Oszczędzasz <strong>{(selectedAttraction.regularPrice - selectedAttraction.groupPrice) * totalOccupiedSpots} zł</strong> łącznie!
                </p>
              )}
            </div>
          )}
        ...
        <Button
          ...
          disabled={!canCreate}
          className="w-full bg-gradient-to-r from-blue-600 to-cyan-600 text-white h-14 shadow-lg disabled:opacity-50"
        >
